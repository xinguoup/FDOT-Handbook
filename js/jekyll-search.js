!function e(t, n, r) {
    ...
    2: [function (require, module) {
        function FuzzySearchStrategy() {
            ...
            self.matches = function (string, crit) {
                return typeof string != "string" ? !1 : (string = string.trim(), !!string.match(createFuzzyRegExpFromString(crit)));
            };
        }
    ...
    3: [function (require, module) {
        function LiteralSearchStrategy() {
            ...
            var self = this;
            self.matches = function (string, crit) {
                return typeof string != "string" ? !1 : (string = string.trim(), doMatch(string, crit));
            };
        }
        ...
        module.exports = function (_store) {
            ...
            function addArray(data) {
                ...
                for (var i = 0; i < data.length; i++) {
                    if (isObject(data[i])) added.push(addObject(data[i]));
                }
                return added;
            }
            ...
            function SimpleJekyllSearch() {
                ...
                function registerInput() {
                    opt.searchInput.addEventListener("keyup", function (e) {
                        if (e.target.value.length === 0) emptyResultsContainer();
                        else render(searcher.search(store, e.target.value));
                    });
                }

                function render(results) {
                    emptyResultsContainer();
                    if (results.length === 0) appendToResultsContainer(opt.noResultsText);
                    else {
                ...
                self.init = function (_opt) {
                    ...
                    assignOptions(_opt);
                    if (isJSON(opt.dataSource)) initWithJSON(opt.dataSource);
                    else initWithURL(opt.dataSource);
                }
            }
